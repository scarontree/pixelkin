# AGENTS.md — DesktopPet macOS Native
...
- ⬚ 控制面板中的设置页面（SettingsView 骨架未创建）
- ⬚ 聊天 / 角色 / Function Calling（Data 层已占位，功能未实现）
- ⬚ Rive 引擎实际接入
...
**当前实现**：动态 `ignoresMouseEvents` + 全局/本地鼠标位置监听。
...
3. 光标在宠物实体矩形热区（±8pt 容错）→ `panel.ignoresMouseEvents = false`
4. 光标在透明区域 → `panel.ignoresMouseEvents = true`
5. 拖拽过程中强制 `ignoresMouseEvents = false`
...
├── DesktopPet/
│   ├── App/
│   │   ├── DesktopPetApp.swift           # @main 入口 + MenuBarExtra + Window
...
│   │       └── SettingsStore.swift       # UserDefaults 封装（选中皮肤 + 可见性）
│   ├── Infrastructure/
│   │   ├── Windowing/
...
enum PetBehaviorState: String, Codable {
    case idle, walk, drag, fall, sleep, sit
}
...
```
States: idle, walk, drag, fall, sleep, sit
...
sit → idle        (10~20s 后站起)
sit → fall        (底下的窗口被关闭)
walk → fall       (走到窗口边缘且没转向，50% 概率掉下去)

Walking surfaces: ground（屏幕底部）/ windowTop（其他窗口顶部）
宠物可在窗口顶部来回行走，到达窗口边缘时 50% 转向 / 50% 掉落
```
...
```json
// ~/Library/Application Support/DesktopPet/Skins/{skinID}/phrases.json
{
...
| 数据 | 位置 |
|---|---|
| 气泡触发规则 | `~/Library/Application Support/DesktopPet/bubble_rules.json` |
| 角色语录本 | `~/Library/Application Support/DesktopPet/Skins/{skinID}/phrases.json` |
...
| 用户导入皮肤 | `~/Library/Application Support/DesktopPet/Skins/{skinID}/` | manifest.json + 资源文件 |
...
| Section | 名称 | 状态 |
|---|---|---|
| `.chat` | 聊天 | 占位（placeholder） |
| `.personas` | 角色 | 占位（placeholder） |
| `.skins` | 皮肤 | ✅ 已实现（SkinGalleryView） |
| `.settings` | 设置 | 占位（placeholder） |
